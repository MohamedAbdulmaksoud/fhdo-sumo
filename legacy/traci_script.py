...
def park_vehicle(vehicle_id):
    ...
    if nearest_parking:
        try:
            ...
            edge_id = traci.lane.getEdgeID(lane_id)
            traci.vehicle.rerouteParkingArea(vehicle_id, nearest_parking)
        except traci.exceptions.TraCIException as e:
            print(f"Error rerouting vehicle {vehicle_id} to parking area {nearest_parking}: {e}")
            return False
                ...
                if current_lane_id == lane_id:
                    ...
                    if vehicle_count < capacity:
                        try:
                            ...
                            return False
                        print(f"Vehicle {vehicle_id} successfully parked at {nearest_parking}.")
                        return True
                    else:
                        ...
                        # Print the parking area ID and occupancy
                        print(f"Vehicle {vehicle_id} arrived at parking area {nearest_parking}, but it was full.")
                        print(f"Parking area ID: {nearest_parking}")
                        print(f"Occupancy: {vehicle_count}/{capacity}")
                        ...
                        if nearest_parking:
                            try:
                                ...
                                print(f"Error rerouting vehicle {vehicle_id} to next nearest parking area {nearest_parking}: {e}")
                                return False
                        else:
                            return False
        except traci.exceptions.TraCIException as e:
...
def main():
    ...
    while traci.simulation.getMinExpectedNumber() > 0:
        ...
        if park_vehicle(tracked_vehicle):
            end_time = traci.simulation.getTime()
            trip_time = end_time - vehicle_data[tracked_vehicle]['start_time']
            average_speed = traci.vehicle.getDistance(tracked_vehicle) / trip_time
            co2_consumption = vehicle_data[tracked_vehicle]['co2']

            print(f"Vehicle {tracked_vehicle} parked")
            ...
            vehicle_data[tracked_vehicle]['co2'] += traci.vehicle.getCO2Emission(tracked_vehicle)
            vehicle_data[tracked_vehicle]['distance'] += traci.vehicle.getDistance(tracked_vehicle)
